...
void ButtonTask(TimerHandle_t xTimer) {
  ...
  if (buttonStatus != 0 && buttonStatus != lastButtonPin && lastButtonPin != 0) {
    ...
    if (ButtonPin) {
      if (((lastButtonPin & GPIO_PIN_8) == GPIO_PIN_8) && lastButtonPin == GPIO_PIN_8) {
        ButtonPin(ButtonPinCallType::LeftButtonPressed);
      } else if ((lastButtonPin & GPIO_PIN_9) == GPIO_PIN_9 && lastButtonPin == GPIO_PIN_9) {
        ButtonPin(ButtonPinCallType::RightButtonPressed);
      } else if ((lastButtonPin & GPIO_PIN_8) == GPIO_PIN_8 && (lastButtonPin & GPIO_PIN_9) == GPIO_PIN_9) {
        ButtonPin(ButtonPinCallType::DoubleButtonPressed);
      }
  ...
  if (buttonStatus == lastButtonPin && lastButtonPin != 0) {
    ...
    if (longPressNum == 6) {
      // 长按事件
      if ((buttonStatus & GPIO_PIN_8) == GPIO_PIN_8 && buttonStatus == GPIO_PIN_8) {
        if (ButtonPin) {
          ButtonPin(ButtonPinCallType::LeftButtonLongPress);
        }
      } else if ((buttonStatus & GPIO_PIN_9) == GPIO_PIN_9 && buttonStatus == GPIO_PIN_9) {
        if (ButtonPin) {
          ButtonPin(ButtonPinCallType::RightButtonLongPress);
        }
      } else if ((buttonStatus & GPIO_PIN_8) == GPIO_PIN_8 && (buttonStatus & GPIO_PIN_9) == GPIO_PIN_9) {
        if (ButtonPin) {
          ButtonPin(ButtonPinCallType::DoubleButtonLongPress);
        }
      }
      isTrigger = true;
      return;
...
void ButtonPinSendWhite(ButtonPinCallType type) {
  // 在主页双键长按
  if (type != ButtonPinCallType::DoubleButtonLongPress || g_sysCtx->Apps.Status != APPID_DESKTOP) {
    return;
  }
  // 强制刷白进入存放模式
...
void ButtonPinInit() {
  ...
  RegisterButtonPinCall(ButtonPinSendWhite);
  // 设置扫描定时器,200ms扫描一次
