...
int BidirectionalPacketBasedChannel::process_packet(uint8_t* buffer, size_t length)
{
    ...
    if (packageCount & 0x8000) // BULK Packet
    {
        ...
        if (needResponse)
        {
            ...
            uint32_t n = requestedLength / USBD_CUSTOMHID_OUTREPORT_BUF_SIZE;
            ...
            while (n--)
            {
                ...
                addressOffset += USBD_CUSTOMHID_OUTREPORT_BUF_SIZE;
            }

            uint32_t lastPacketLength = requestedLength % USBD_CUSTOMHID_OUTREPORT_BUF_SIZE;
        ...
        } else
        {
            ...
            OnBulkPacket(buffer, length,
                         addressOffsetCount * (USBD_CUSTOMHID_OUTREPORT_BUF_SIZE-2),
                         0);
        }
    } else // RPC Packet
    {
